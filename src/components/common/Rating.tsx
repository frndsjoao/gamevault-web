...
export default function Rating({ rating, size = 16, className = "", enabled = false, onChange }: RatingProps) {
  ...
  return (
    <div className={`flex flex-row items-center space-x-3 ${className}`}>
      <div className="flex flex-row items-center gap-0.5" onMouseLeave={handleMouseLeave}>
            ...
            >
              {isHalf ? (
                ...
                <Icon
                  name="star"
                  size={size}
                  className={isFilled ? "fill-text-light text-text-light" : "fill-none !text-gray-600"}
                />
              )}
            </div>
          );
      ...
      <span className="pt-0.5 text-xs text-text-medium">{displayRating.toFixed(1)}</span>
    </div>
  );
}
